...
while True:
    ...
    if time.time() - last > .5:
        offset += 1
        print('{0} - {1}'.format(offset % 4, int(math.floor(offset / 4) % 2)))
        last = time.time()
        num = tick # int(random.random() * 8)
        perc = (math.floor(offset / 4) % 2) * .8
        pwm.set_pwm(offset % 4, 0, int(perc * servo_diff + servo_min))
        pwm.set_pwm(offset % 4 + 4, 0, int(perc * servo_diff + servo_min))
